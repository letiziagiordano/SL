...
L'obiettivo dell'analisi è stato analizzare il turismo in Italia in un particolare periodo dell'anno: il mese di Gennaio. Oltre a essere il primo mese dell'anno, è caratterizzato dal particolare clima freddo, e poichè include alcune festività, come Capodanno e l'Epifania, permette una migliore organizzazione delle vacanze. 
...
Si osserva che Bolzano nel 2016 è la provincia con il maggior numero di arrivi in assoluto negli ultimi 9 anni. La distribuzione percentuale degli arrivi in questo anno è descritta dalla seguente mappa.
...
Considerato che il mese preso in esame è del periodo invernale, si nota una maggiore affluenza degli arrivi nell'Italia Settentrionale. 
...
ggplot(data = demo) +
  ...
  annotate("text", x = 5, y = 9, label = paste(round(tra[4,9]/som*100,0),"%",sep=""))+theme(plot.background =element_rect(fill="#F8F8FF"))
```
...
ggplot(data = demo) +
  ...
  annotate("text", x = 8, y = 11, label = paste(round(tra[4,8]/tra[9,8]*100,0),"%",sep=""))+theme(plot.background =element_rect(fill="#F8F8FF"))
```
